...
def checkpoint_metadata(name: str, path: Path, device: torch.device) -> Dict[str, object]:
    ...
    model_cfg = config.get("model3d", {})
    geometry_prior_weight = ckpt.get("geometry_prior_weight", model_cfg.get("jepa_geometry_prior_weight"))
    geometry_prior_log_std = ckpt.get("geometry_prior_log_std", model_cfg.get("jepa_geometry_prior_log_std"))
    return {
        "variant": name,
        "checkpoint": str(path),
        ...
        "geometry_prior_log_std": numeric(geometry_prior_log_std),
...
def markdown_report(rows: List[Dict[str, object]], claims: Dict[str, object]) -> str:
    lines = [
        ...
        "## Variants",
        "",
        "| variant | EMA | SIGReg | visual inv | geom teacher | geom prior | prior log std | visual geom | target encoder | belief head | structured | RGB-D | checkpoint |",
        "| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | ---: | ---: | --- |",
    ]
    seen = set()
    for row in rows:
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    format_float(row.get("geometry_prior_log_std")),
