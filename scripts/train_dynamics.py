...
from src.train.losses import state_rollout_loss
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train state-space dynamics model.")
    parser.add_argument("--config", type=str, default="configs/default.yaml")
    return parser.parse_args()


@torch.no_grad()
def evaluate_val(model: ObjectCentricDynamics, loader, device: torch.device) -> dict[str, float]:
    model.eval()
    losses = []
    rmses = []
    for batch in loader:
        ...
        pred = model(init_state, object_mask, occluders, horizon=future_state.shape[1])
        loss_dict = state_rollout_loss(pred, future_state, future_mask)
        losses.append(float(loss_dict["total"].item()))
        rmse = torch.sqrt(((pred[..., 0:2] - future_state[..., 0:2]) ** 2).sum(dim=-1))
    ...
    return {
        ...
        "val_rollout_rmse": float(sum(rmses) / max(len(rmses), 1)),
    }
...
def main() -> None:
    ...
    epochs = int(train_cfg["epochs_dynamics"])
    grad_clip = float(train_cfg["grad_clip"])

    run_dir = init_run_dir(config["project"]["output_root"], "train_dynamics", config)
