...
from .scene_generator3d import Scene3DConfig, SyntheticScene3DGenerator
...
def scene3d_config_from_data_cfg(data_cfg: Dict[str, Any]) -> Scene3DConfig:
    subset = {k: v for k, v in data_cfg.items() if k in SCENE3D_CONFIG_KEYS}
    return Scene3DConfig(**subset)


class SyntheticScene3DDataset(Dataset):
    def __init__(self, manifest_path: str | Path, data_cfg: Dict[str, Any]):
        self.manifest_path = Path(manifest_path)
        self.rows = read_manifest(self.manifest_path)
        self.scene_cfg = scene3d_config_from_data_cfg(data_cfg)
        self.generator = SyntheticScene3DGenerator(self.scene_cfg)
...
def collate_scenes3d(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    output: Dict[str, Any] = {}
    tensor_keys = {
        "obs_frames",
        "future_frames",
...
def split_paths3d(manifest_dir: str | Path) -> Tuple[Path, Path]:
